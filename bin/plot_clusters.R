...
for (l in lines) {
	tok=strsplit(l, "=")
	if (tok[[1]][1]=="class") {
		class=strsplit(tok[[1]][2], ",")
	} else if (tok[[1]][1]=="timepoints") {
		tps=strsplit(tok[[1]][2], ",")
	}
}
...
data$Samples<-factor(data$Samples, levels=unlist(class))
data$TimePoint<-factor(data$TimePoint, levels=unlist(tps))
...
for (clid in clids) {
	...
	min_y<-min(data_clid$Min)
	max_y<-max(data_clid$Max)

	pdf(paste(outdir, "/cluster_", clid, "_repr.pdf", sep=""), bg="transparent", height=5, width=6)
	title=paste("Cluster ", clid, cluster_type, sep=" ") 

	p<-ggplot(data=data_clid, aes(x=TimePoint, y=Value, group=Samples, color=Samples))+
